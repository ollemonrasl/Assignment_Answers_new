  ...
  gendict.each {|id,embl|
    ...
    embl.features do |feature|
      ...
      if feature.locations.to_s.match(forw) # FOR EXONS IN THE FORWARD STRAND
          ...
          exons eq = seq.subseq(ini,fin) # CUT SEQUENCE INTO EXON
          ...
          unless match_seq(exonseq).nil?
            ...
            posit.each {|stenpair|
              ...
              start = stenpair.split("::")[0]
              ending = stenpair.split("::")[1]
              genen = embl.sv().split(":")[4].to_i - start.to_i
              genen = genst + 5
              newf = new_feature(start,ending,embl.sv().split(":")[2],genst,genen,"-",note)
              embl.features << newf}
          else
...
gff3_file = File.open("gff3_gene_report.gff3","a+")
...
gff3_file = File.open("gff3_chromosome_report.gff3","a+")
gff3_file.puts "##gff-version 3"
source = "BioRuby"
type = "cttctt_repeat"
score = "."
...
gendict.each {|seq,embl|
  embl.features {|feature|
    ...
    start = feature.assoc["start"].to_i
    ending = feature.assoc["ending"].to_i
    genestart = feature.assoc["genestart"].to_i + start
    geneend = feature.assoc["geneend"].to_i + ending
    strand = feature.assoc["strand"]
    attrib = feature.assoc["attrib"]
